...
export type SandboxWorkerDeployResult =
  | { ok: true; sandboxId: string; steps: SandboxStepLog[] }
  | { ok: false; error: string; sandboxId: string | null; steps: SandboxStepLog[] };

/**
 * Runs `ntn workers deploy` inside a Vercel Sandbox VM.
 */
export async function deployNotionWorkerInSandbox(): Promise<SandboxWorkerDeployResult> {
  ...
  try {
    ...
    await sandbox.writeFiles([
      {
        path: `${SANDBOX_WORKER_ROOT}/package.json`,
        content: Buffer.from(files.pkg, "utf8"),
      },
      {
        path: `${SANDBOX_WORKER_ROOT}/tsconfig.json`,
        content: Buffer.from(files.tsconfig, "utf8"),
      },
      {
        path: `${SANDBOX_WORKER_ROOT}/src/index.ts`,
        content: Buffer.from(files.idx, "utf8"),
      },
    ]);
    ...
    await runStep(
      ...
      await sandbox.runCommand({
        cmd: "ntn",
        args: ["workers", "deploy", "--name", "notionchella-festival-worker"],
        cwd: SANDBOX_WORKER_ROOT,
        env: {
    ...
    return { ok: true, sandboxId: sandbox.sandboxId, steps };
  } catch (err) {
...
export function canDeployWorkerInSandbox(): boolean {
  return Boolean(
    process.env.NOTION_API_TOKEN?.trim() &&
      process.env.NOTION_WORKSPACE_ID?.trim(),
  );
}
